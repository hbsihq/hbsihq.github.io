...
<?php if (!empty($this->options->sidebarBlock) && in_array('ShowRecentPosts', $this->options->sidebarBlock)): ?>
    <div class="widget widget-tops">
        <ul class="widget-nav"><li class="active">网站源码</li></ul>
...
<?php if (!empty($this->options->sidebarBlock) && in_array('ShowCategory', $this->options->sidebarBlock)): ?>
    <?php
        ...
        for($i=1;$i<=$maxi;$i++){
            $icss=($i%2)+1;
            $tmp='<div class="widget widget_ui_textasb"><a class="style0'.$icss.'" href="'.$sad1[$i*3-3].'" target="_blank"><strong>主机推荐</strong><h2>'.$sad1[$i*3-2].'</h2><p>'.$sad1[$i*3-1].'</p></a></div>';
            echo $tmp;
        }
    ?>
<?php endif; ?>
